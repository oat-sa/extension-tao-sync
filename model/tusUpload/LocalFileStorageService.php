...
class LocalFileStorageService extends ConfigurableService
{
    ...
    public function writeChunk($meta)
    {
        ...
        //$output = $this->open($this->getFilePath($meta['fileName'] . $meta['key']), self::APPEND_BINARY); //TODO: revert comment
        $output = $this->open('/srv/tao/act2data/' . $meta['fileName'] . $meta['key'], self::APPEND_BINARY);
        ...
        try {
            ...
            syslog(1, filesize('/srv/tao/act2data/' . $meta['fileName'] . $meta['key']) . '+++' . $this->offset);
            $this->close($input);
            $this->close($output);
            syslog(1, filesize('/srv/tao/act2data/' . $meta['fileName'] . $meta['key']) . '===' . $this->offset);
        }
    ...
    protected function write($handle, $data, $length = null)
    {
        ...
        if ($bytesWritten !== strlen($data)) {
            syslog(1, '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!' . $bytesWritten . '-' . strlen($data));
        }
