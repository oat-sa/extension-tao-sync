...
use oat\taoSync\model\tusUpload\events\UploadCompleteEvent;
use oat\taoSync\model\tusUpload\events\UploadCreatedEvent;
use oat\taoSync\model\tusUpload\events\UploadProgressEvent;
...
class TusUploadServerService extends AbstractTus implements TusUploadServerServiceInterface
{
    ...
    public function getUploadKey()
    {
        if (empty($this->uploadKey)) {
            $this->uploadKey = $this->getHeaderItem('Upload-Key');
        }
    ...
    protected function handlePost()
    {
        ...
        $maxUploadSize = $this->getOption(self::OPTION_UPLOAD_MAX_SIZE);
        if (!($maxUploadSize > 0 && $this->getHeaderItem('Upload-Length') > $maxUploadSize)) {
            return $this->prepareResponseData(null, self::HTTP_REQUEST_ENTITY_TOO_LARGE);
        }
        $data = [
            'name'        => $fileName,
            'offset'      => 0,
            'size'        => $this->getHeaderItem('Upload-Length'),
     ...
     *
     * @throws \oat\oatbox\service\exception\InvalidServiceManagerExceptionhandleHead
     */
    protected function handlePatch()
    {
        ...
        try {
            ...
            if ($offset === $meta['fileSize']) {
                ...
                $responseData = $this->prepareResponseData(null, self::HTTP_OK, [
    ...
    protected function verifyPatchRequest($meta)
    {
        ...
        $uploadOffset = $this->getHeaderItem('upload-offset');
        if ($uploadOffset && $uploadOffset !== (string)$meta['offset']) {
            $status = self::HTTP_CONFLICT;
        }
        $contentType = $this->getHeaderItem('Content-Type');
    ...
    {
        $checksumHeader = $this->getHeaderItem('Upload-Checksum');
        if (!empty($checksumHeader)) {
            list($checksumAlgorithm, $checksum) = explode(' ', $checksumHeader);
    ...
    {
        $requestedValue = '';
        $uploadMetaData = $this->getHeaderItem('Upload-Metadata');
        if (!empty($uploadMetaData)) {
            ...
            foreach ($uploadMetaDataChunks as $chunk) {
                ...
                if ($key === $requestedKey) {
                    $requestedValue = base64_decode($value);
                    break;
                }
            }
        }
    ...
    protected function getHeaderItem($key)
    {
