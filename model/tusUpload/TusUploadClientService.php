...
use oat\taoSync\model\tusUpload\Clients\ClientAdapterInterface;
...
class TusUploadClientService extends AbstractTus implements TusUploadClientServiceInterface
{
    ...
    public function upload($filePath = '', $url = '', $key = '')
    {
        ...
        try {
            ...
            $offset = isset($responseData['headers']['Upload-Offset']) ? (int)$responseData['headers']['Upload-Offset'] : 0;
        ...
        while ($fileChunk = $fs->readChunk($filePath, $offset, $this->getOption(self::OPTION_CHUNK_SIZE))) {
            ...
            $data = $this->sendPatchRequest($offset, $fileChunk, $key);
            $offset = $data['headers']['Upload-Offset'];
        }
    }
    ...
    protected function create($key)
    {
        ...
        $responseData = $this->getClient()->post($this->getUrl(), ['headers' => $headers]);
    ...
    {
        $responseData = $this->getClient()->head($this->getUrl(), ['headers' => ['Upload-Key' => $key]]);
        $statusCode = $responseData['statusCode'];
        return $responseData;
    }
    ...
    protected function sendPatchRequest($offset, $fileChunk, $key)
    {
        ...
        try {
            $responseData = $this->getClient()->patch($this->getUrl(), [
                'body'    => $fileChunk,
                'headers' => $headers,
            ]);
            if($responseData['statusCode']!=self::HTTP_OK){
