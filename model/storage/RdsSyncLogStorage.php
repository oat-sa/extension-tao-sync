...
class RdsSyncLogStorage extends ConfigurableService implements SyncLogStorageInterface
{
    ...
    public function create(SyncLogEntity $entity)
    {
        $this->getPersistence()->insert(
            ...
            [
                ...
                self::COLUMN_CLIENT_ID => $entity->getClientId(),
    ...
    public function update(SyncLogEntity $entity)
    {
        ...
        $result = $queryBuilder->update(self::TABLE_NAME)
            ->set(self::COLUMN_STATUS, $entity->getStatus())
            ->set(self::COLUMN_DATA, json_encode($entity->getData()))
            ->set(self::COLUMN_REPORT, json_encode($entity->getReport()))
            ->set(self::COLUMN_FINISHED_AT, $entity->getFinishTime()->format(SyncLogEntity::DATE_TIME_FORMAT))
            ->where(self::COLUMN_ID . ' = ' . $queryBuilder->createNamedParameter($entity->getId()));
    ...
    public function getBySyncIdAndClientId($syncId, $clientId)
    {
        ...
        $queryBuilder->select('*')
            ...
            ->andWhere(self::COLUMN_CLIENT_ID . ' = ' . $queryBuilder->createNamedParameter($clientId, \PDO::PARAM_STR));
