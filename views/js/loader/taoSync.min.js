...
total:0},config.aggregatedData[execution.context_id].label=execution.label,config.aggregatedData[execution.context_id].total++}),this.config=config}).on("render",function(){var self=this;$container.find(".cancel-button").on("click",function(e){e.preventDefault(),self.trigger("terminationCanceled"),self.destroy()}),$container.find(".force-terminate-button").on("click",function(e){var requestData;e.preventDefault(),(requestData={executionsId:config.activeExecutions},requestData[config.csrfToken.name]=config.csrfToken.token,request(config.terminateUrl,requestData,"POST")).then(function(response){self.trigger("terminationSucceeded"),self.destroy()}).catch(
function(error){self.trigger("terminationFailed")})}),self.trigger("dialogRendered")}).init(config)}}),define("taoSync/controller/synchronizer/index",["jquery","lodash","moment","core/dataProvider/request","util/url","core/taskQueue/taskQueueModel","layout/loading-bar","taoSync/component/terminateExecutions/terminateExecutions"],function($,_,moment,request,urlHelper,taskQueueModelFactory,loadingBar,terminateExecutionsDialogFactory){"use strict";var tq_api="TaskQueueWebApi",tq_ext="taoTaskQueue",webservices={get:urlHelper.route("get",tq_api,tq_ext),archive:urlHelper.route("archive",tq_api,tq_ext),all:urlHelper.route("getAll",tq_api,tq_ext),
download:urlHelper.route("download",tq_api,tq_ext),lastTask:urlHelper.route("lastTask","Synchronizer","taoSync"),activeSessions:urlHelper.route("activeSessions","Synchronizer","taoSync"),terminateExecutions:urlHelper.route("terminateExecutions","TerminateExecution","taoSync")};return{start:function(){var _msg,$container=$("#tao-sync-container"),$syncForm=$container.find("form.sync-form"),$syncFormFields=($container.find("form.sync-form"),$syncForm.find("input, select")),$launchButton=$syncForm.find('button[data-control="launch"]'),$spinner=$syncForm.find(".feedback-info .icon-loop"),timeFields={$enqueued:$syncForm.find(".enqueue-time"),
