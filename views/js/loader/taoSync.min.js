...
,disablePaginations($elt.paginations),$elt.trigger("query."+ns,[ajaxConfig]),options.status&&$elt.find(".loading").removeClass("hidden"),ajaxConfig.url?$$1.ajax(ajaxConfig).done(function(response){self._render($elt,response),loadingBar.stop()}).fail(function(response,option,err){var requestErr=httpErrorParser.parse(response,option,err);logger.error(requestErr.message),requestErr.code=response.status,enablePaginations(this.paginations),$elt.trigger("error."+ns,[requestErr]),self._render($elt,{}),loadingBar.stop()}):(self._render($elt,options.data),loadingBar.stop())},_render:function($elt,dataset){var $rendering,$statusEmpty,$statusAvailable,$statusCount,
$sortBy,$sortElement,$checkAll,$checkboxes,$rows,amount,transforms,self=this,options=_.cloneDeep($elt.data(dataNs)),$massActionBtns=$$1(),model=[],join=function(input){return"object"!=typeof input?input:input.join(", ")};dataset=dataset||{},$elt.trigger("beforeload."+ns,[_.cloneDeep(dataset)]),_.forEach(options.model,function(field,key){options.filter||(field.filterable=!1),_.isUndefined(field.order)&&(field.order=key+1),field.filterable&&"object"!=typeof field.filterable&&(field.filterable={placeholder:__("Filter")}),field.transform&&(field.transform=_.isFunction(field.transform)?field.transform:join),void 0===field.visible?model.push(field
):"function"==typeof field.visible&&field.visible()?model.push(field):!0===field.visible&&model.push(field)}),model.sort(function(a,b){return a.order-b.order}),options.sortby&&(options=this._sortOptions($elt,options.sortby,options.sortorder,options.sorttype)),_.some(options.model,"transform")&&(transforms=_.where(options.model,"transform"),_.forEach(dataset.data,function(row,index){_.forEach(transforms,function(field){row[field.id]=field.transform(row[field.id],row,field,index,dataset.data)})})),options.model=model,$rendering=$$1(function(data,options){var html=Template(data,options);return html}({options:options,dataset:dataset})),_.forEach(dataset.readonly
,function(values,id){if(!0===values)$$1('[data-item-identifier="'+id+'"] button',$rendering).addClass("disabled");else if(values&&"object"==typeof values)for(var action in values)values.hasOwnProperty(action)&&!0===values[action]&&$$1('[data-item-identifier="'+id+'"] button.'+action,$rendering).addClass("disabled")});function attachActionListeners(actions){_.forEach(actions,function(action,name){var css;_.isFunction(action)||(name=action.id||name,action=action.action||function(){}),css="."+name,$rendering.off("click",css).on("click",css,function(e){var $btn=$$1(this);if(e.preventDefault(),!$btn.hasClass("disabled")){var identifier=$btn.closest(
"[data-item-identifier]").data("item-identifier");action.apply($btn,[identifier,_.first(_.where(dataset.data,{id:identifier}))])}})})}if(options.actions&&attachActionListeners(options.actions),_.some(options.model,"type")){var types=_.where(options.model,"type");_.forEach(types,function(field){"actions"===field.type&&field.actions&&attachActionListeners(field.actions)})}function renderPagination($container,mode){return paginationComponent({mode:mode,activePage:dataset.page,totalPages:dataset.total}).on("change",function(){self._setPage($elt,this.getActivePage())}).on("prev",function(){$elt.trigger("backward."+ns)}).on("next",function(){$elt.trigger(
"forward."+ns)}).render($container)}_.forEach(options.tools,function(action,name){var css,massAction=!0;_.isFunction(action)||(name=action.id||name,massAction=action.massAction,action=action.action||function(){}),css=".tool-"+name,massAction&&($massActionBtns=$massActionBtns.add($rendering.find(css))),$rendering.off("click",css).on("click",css,function(e){var $btn=$$1(this);e.preventDefault(),$btn.hasClass("disabled")||action.apply($btn,[self._selection($elt)])})}),_.forEach(options.listeners,function(callback,event){var ev=[event,ns].join(".");$elt.off(ev).on(ev,callback)}),$elt.paginations=[],"none"!==options.paginationStrategyTop&&$elt.paginations.push(
renderPagination($$1(".datatable-pagination-top",$rendering),options.paginationStrategyTop)),"none"!==options.paginationStrategyBottom&&$elt.paginations.push(renderPagination($$1(".datatable-pagination-bottom",$rendering),options.paginationStrategyBottom)),disablePaginations($elt.paginations),$rows=$rendering.find("tbody tr"),$sortBy=$rendering.find("th [data-sort-by]"),$sortElement=$rendering.find('[data-sort-by="'+options.sortby+'"]'),$checkAll=$rendering.find("th.checkboxes input"),$checkboxes=$rendering.find("td.checkboxes input"),options.rowSelection&&($$1("table.datatable",$rendering).addClass("hoverable"),$rendering.on("click","tbody td",function(e){
if($$1(e.target).hasClass("checkboxes")||$$1(e.target).hasClass("actions"))return!1;var currentRow=$$1(this).parent();$rows.removeClass("selected"),currentRow.toggleClass("selected"),$elt.trigger("selected."+ns,_.where(dataset.data,{id:currentRow.data("item-identifier")}))})),$sortBy.on("click keyup",function(e){var column,type;"keyup"===e.type&&13!==e.keyCode||(e.preventDefault(),column=$$1(this).data("sort-by"),type=$$1(this).data("sort-type"),self._sort($elt,column,void 0,type))}),options.filter&&(self._getFilterStrategy($elt).render($rendering,options),_.forEach($$1(".filter",$rendering),function(filter){var $filter=$$1(filter),$filterBtn=$$1("button",
$filter),$filterInput=$$1("select, input",$filter);$filterInput.is("select")?$filterInput.on("change",function(){self._filter($elt,$filter)}):($filterBtn.off("click").on("click",function(e){e.preventDefault(),self._filter($elt,$filter)}),$filterInput.off("keypress").on("keypress",function(e){13===e.which&&(e.preventDefault(),self._filter($elt,$filter))}))})),$checkAll.click(function(){this.checked?($checkAll.prop("checked",!0),$checkboxes.prop("checked",!0)):($checkAll.prop("checked",!1),$checkboxes.prop("checked",!1)),$massActionBtns.length&&$massActionBtns.toggleClass("invisible",!$checkboxes.filter(":checked").length),$elt.trigger("select."+ns)}),
...
"use strict";_=_&&_.hasOwnProperty("default")?_.default:_,Promise=Promise&&Promise.hasOwnProperty("default")?Promise.default:Promise,uuid=uuid&&uuid.hasOwnProperty("default")?uuid.default:uuid;var commonjsGlobal="undefined"!=typeof globalThis?globalThis:"undefined"!=typeof window?window:"undefined"!=typeof global?global:"undefined"!=typeof self?self:{};var module,knownStores,idbstore=(function(module){var definition,global;definition=function(){function defaultErrorHandler(error){throw error}function defaultSuccessHandler(){}function IDBStore(kwArgs,onStoreReady){for(var key in void 0===onStoreReady&&"function"==typeof kwArgs&&(onStoreReady=kwArgs),
...
;return requestData[config.csrfToken.name]=config.csrfToken.token,request(config.terminateUrl,requestData,"POST")}().then(function(response){self.trigger("terminationSucceeded"),self.destroy()}).catch(function(error){self.trigger("terminationFailed")})}),self.trigger("dialogRendered")}).init(config)}}),define("css!ui/dashboard/css/dashboard",[],function(){}),define("ui/dashboard",["i18n","lodash","ui/component","handlebars","lib/dompurify/purify","css!ui/dashboard/css/dashboard.css"],function(__,_,component,Handlebars,DOMPurify,dashboard_css){"use strict";function Helpers0(hb){hb.registerHelper("__",function(key){return __(key)}),hb.registerHelper(
...
}),hb.registerHelper("for",function(startIndex,stopIndex,increment,options){var ret="";startIndex=parseInt(startIndex),stopIndex=parseInt(stopIndex),increment=parseInt(increment);for(var i=startIndex;i<stopIndex;i+=increment)ret+=options.fn(_.extend({},this,{i:i}));return ret}),hb.registerHelper("equal",function(var1,var2,options){return var1==var2?options.fn(this):options.inverse(this)}),hb.registerHelper("property",function(name,context){return context[name]||""}),hb.registerHelper("includes",function(haystack,needle,options){if(_.contains(haystack,needle))return options.fn(this)})}__=__&&__.hasOwnProperty("default")?__.default:__,_=_&&_.hasOwnProperty(
"default")?_.default:_,component=component&&component.hasOwnProperty("default")?component.default:component,Handlebars=Handlebars&&Handlebars.hasOwnProperty("default")?Handlebars.default:Handlebars,DOMPurify=DOMPurify&&DOMPurify.hasOwnProperty("default")?DOMPurify.default:DOMPurify,Helpers0.__initialized||(Helpers0(Handlebars),Helpers0.__initialized=!0);var Template=Handlebars.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var helper,buffer="",escapeExpression=this.escapeExpression
;return buffer+='<div class="readiness-dashboard-container">\n    <h3 class="dashboard-header">',buffer+=escapeExpression((helper=helpers.headerText)?helper.call(depth0,{hash:{},data:data}):"function"==typeof(helper=depth0&&depth0.headerText)?helper.call(depth0,{hash:{},data:data}):helper)+'</h3>\n    <section>\n        <div class="dashboard-loading" style="display: none;">\n            <p class="dashboard-text">\n                ',buffer+=escapeExpression((helper=helpers.loadingText)?helper.call(depth0,{hash:{},data:data}):"function"==typeof(helper=depth0&&depth0.loadingText)?helper.call(depth0,{hash:{},data:data}):helper
)+'\n            </p>\n        </div>\n        <div class="dashboard-metrics_container"></div>\n        <div class="dashboard-warning" style="display: none;">\n            <p class="dashboard-text">\n                ',buffer+=escapeExpression((helper=helpers.warningText)?helper.call(depth0,{hash:{},data:data}):"function"==typeof(helper=depth0&&depth0.warningText)?helper.call(depth0,{hash:{},data:data}):helper)+"\n            </p>\n        </div>\n    </section>\n</div>\n"});function dashboardTpl(data,options,asString){var html=Template(data,options);return html}Helpers0.__initialized||(Helpers0(Handlebars),Helpers0.__initialized=!0)
;var Template$1=Handlebars.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var stack1,helper,buffer="",functionType="function",escapeExpression=this.escapeExpression,self=this;function program2(depth0,data){var stack1,helper,buffer="";return buffer+='\n                    <li class="dashboard-metric_info-item">',stack1=(helper=helpers.text)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.text)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+"</li>\n                "}
return buffer+='<ul class="dashboard-metrics_list ',stack1=(helper=helpers.layoutType)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.layoutType)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'">\n    ',!(stack1=helpers.each.call(depth0,depth0&&depth0.data,{hash:{},inverse:self.noop,fn:self.program(1,function(depth0,data){var stack1,helper,buffer="";return buffer+='\n        <li class="dashboard-metric">\n            <h4 class="dashboard-metric_title">',stack1=(helper=helpers.title)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.title)===functionType?helper.call(
depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'</h4>\n            <div class="dashboard-metric_score-container">\n                <div class="dashboard-metric_score score-',stack1=(helper=helpers.state)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.state)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1)+'" style="width: ',stack1=(helper=helpers.score)?helper.call(depth0,{hash:{},data:data}):typeof(helper=depth0&&depth0.score)===functionType?helper.call(depth0,{hash:{},data:data}):helper,buffer+=escapeExpression(stack1
)+'%;"></div>\n            </div>\n            <ul class="dashboard-metric_info">\n                ',!(stack1=helpers.each.call(depth0,depth0&&depth0.info,{hash:{},inverse:self.noop,fn:self.program(2,program2,data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+="\n            </ul>\n        </li>\n    "},data),data:data}))&&0!==stack1||(buffer+=stack1),buffer+="\n</ul>\n"});var defaults={headerText:__("Outlook on the next Synchronization"),loadingText:__("Creating report ..."),warningText:__("Please contact your system administrator."),loading:!1,data:[],scoreState:{error:32,warn:65},layoutType:"tiles"};return function(config){return component({
clearDashboard:function(){this.getElement().find(".dashboard-metrics_container").empty(),this.toggleWarningMessage(!1)},mapScoreToState:function(score){var scoreState=this.config.scoreState;return score>scoreState.warn?"success":score>scoreState.error?"warn":"error"},renderMetrics:function(data){var $listContainer=this.getElement().find(".dashboard-metrics_container"),self=this;if(data&&data.length){_.forEach(data,function(item){item.state=self.mapScoreToState(item.score)}),this.toggleWarningMessage(_.some(data,function(item){return item.score<=self.config.scoreState.warn}));var $metricsList=$(function(data,options){var html=Template$1(data,options)
;return html}({data:data,layoutType:self.config.layoutType}));$listContainer.append($metricsList)}},toggleLoadingBar:function(display){this.getElement().find(".dashboard-loading").toggle(display)},toggleWarningMessage:function(display){this.getElement().find(".dashboard-warning").toggle(display)}},defaults).setTemplate(dashboardTpl).on("init",function(){this.setState("loading",this.config.loading)}).on("render",function(){this.is("loading")?this.toggleLoadingBar(!0):this.renderMetrics(this.config.data)}).init(config)}}),define("taoSync/component/readinessDashboard/readinessDashboard",["i18n","lodash","core/eventifier","core/promise","ui/dashboard",
"core/dataProvider/request","util/url"],function(__,_,eventifier,Promise,dashboard,request,urlHelper){"use strict";return function($container){return eventifier({performChecks:function(){var self=this;new Promise(function(resolve){request(urlHelper.route("getMachineChecks","Synchronizer","taoSync")).then(function(data){resolve(data)})}).then(function(data){self.dashboard.toggleLoadingBar(!1),self.dashboard.renderMetrics(data)})},render:function(){return this.dashboard=dashboard({renderTo:$container,loading:!0,headerText:!1,warningText:!1,layoutType:"list"}),this.performChecks(),this},destroy:function(){return this.dashboard&&this.dashboard.destroy(),
this.trigger("destroy"),this}})}}),define("taoSync/controller/synchronizer/index",["jquery","lodash","moment","core/dataProvider/request","util/url","ui/taskQueue/taskQueueModel","layout/loading-bar","taoSync/component/terminateExecutions/terminateExecutions","taoSync/component/readinessDashboard/readinessDashboard"],function($,_,moment,request,urlHelper,taskQueueModelFactory,loadingBar,terminateExecutionsDialogFactory,readinessDashboardFactory){"use strict";var tq_api="TaskQueueWebApi",tq_ext="taoTaskQueue",webservices={get:urlHelper.route("get",tq_api,tq_ext),archive:urlHelper.route("archive",tq_api,tq_ext),all:urlHelper.route("getAll",tq_api,tq_ext),
download:urlHelper.route("download",tq_api,tq_ext),lastTask:urlHelper.route("lastTask","Synchronizer","taoSync"),activeSessions:urlHelper.route("activeSessions","Synchronizer","taoSync"),terminateExecutions:urlHelper.route("terminateExecutions","TerminateExecution","taoSync")};return{start:function(){var readinessDashboard,_msg,$container=$("#tao-sync-container"),$syncForm=$container.find("form.sync-form"),$syncFormFields=($container.find("form.sync-form"),$syncForm.find("input, select")),$launchButton=$syncForm.find('button[data-control="launch"]'),$spinner=$syncForm.find(".feedback-info .icon-loop"),timeFields={$enqueued:$syncForm.find(".enqueue-time"),
$updated:$syncForm.find(".update-time"),$completed:$syncForm.find(".complete-time")},msg=((_msg={$all:$syncForm.find(".msg")}).$all.each(function(){var $currentMsg=$(this);_msg["$"+$currentMsg.data("type")]=$currentMsg}),_msg),taskQueue=taskQueueModelFactory({url:{get:webservices.get,archive:webservices.archive,all:webservices.all,download:webservices.download},pollSingleIntervals:[{iteration:Number.MAX_SAFE_INTEGER,interval:2e3}],pollAllIntervals:[{iteration:1,interval:8e3},{iteration:0,interval:5e3}]}).on("pollSingleFinished",function(taskId,taskData){"completed"===taskData.status?(setState("success"),updateTime(taskData),setHistoryTime(taskData.updatedAt,
"$completed")):"failed"===taskData.status&&setState("error")}).on("pollSingle",function(taskId,taskData){updateTime(taskData)}).on("error",function(error){error.response&&400<=error.response.errorCode&&error.response.errorCode<500?setState("error",error.response.errorMsg):setState("error")});function getData(){var data={label:"Data Synchronization"};return $syncFormFields.each(function(){data[this.name]=this.value}),data}function toggleLaunchButtonState(){var isValid=!0;$syncFormFields.each(function(){return isValid=this.validity.valid}),isValid?$launchButton.removeAttr("disabled"):$launchButton.attr("disabled","disabled")}function setState(state,message){
if($container.removeClass(function(index,className){return(className.match(/(^|\s)state-\S+/g)||[]).join(" ")}),$spinner["progress"===state?"addClass":"removeClass"]("spinner-icon"),$container.addClass("state-"+state),msg.$all.hide(),message){var statusClassName=".status-"+state;$container.find(statusClassName+" .messages p span:first-child").text(message)}"success"===state||"error"===state?function(){var $dashboardContainer=$container.find("#dashboard-container");readinessDashboard=readinessDashboardFactory($dashboardContainer).render()}():readinessDashboard&&readinessDashboard.destroy()}function updateTime(taskData){setTime(taskData.createdAt,"$enqueued"),
setTime(taskData.createdAt+taskData.createdAtElapsed,"$updated")}function setTime(timestamp,type){timeFields[type]&&timeFields[type].text(moment.unix(timestamp).format("LTS")),msg[type]&&msg[type].show()}function setHistoryTime(timestamp,type){setTime(timestamp,type),$container.addClass("history")}$container.find(".fb-container").removeClass("viewport-hidden"),loadingBar.start(),$syncFormFields.on("keyup paste blur",toggleLaunchButtonState),toggleLaunchButtonState(),$syncForm.on("submit",function(e){var action=this.action;e.preventDefault(),setState("progress"),$container.removeClass("active"),request(webservices.activeSessions).then(function(data){if(
Array.isArray(data.activeSessionsData)&&0<data.activeSessionsData.length){var $terminateActionContainer=$container.find(".terminate-action"),$syncActionContainer=$container.find(".sync-action"),dialogConfig={data:data.activeSessionsData,csrfToken:data.token,terminateUrl:webservices.terminateExecutions};terminateExecutionsDialogFactory($terminateActionContainer,dialogConfig).on("dialogRendered",function(){$syncActionContainer.toggle(),$terminateActionContainer.toggle()}).on("terminationCanceled",function(){$terminateActionContainer.toggle(),setState("form"),$syncActionContainer.toggle()}).on("terminationFailed",function(){}).on("terminationSucceeded",
function(){$terminateActionContainer.toggle(),$syncActionContainer.toggle(),taskQueue.create(action,getData())}).render($terminateActionContainer)}else taskQueue.create(action,getData())}).catch(function(){setState("error")})}),$syncForm.find('button[data-control="close"]').on("click",function(e){e.preventDefault(),setState("form")}),request(webservices.lastTask).then(function(currentTask){if(currentTask&&currentTask.status)switch(currentTask.status){case"failed":setState("error");break;case"completed":setState("form"),updateTime(currentTask),setHistoryTime(currentTask.updatedAt,"$completed");break;default:setState("progress"),updateTime(currentTask),
taskQueue.pollSingle(currentTask.id)}else setState("form");loadingBar.stop()}).catch(function(){setState("error")})}}}),function(c){var d=document,a="appendChild",i="styleSheet",s=d.createElement("style");s.type="text/css",d.getElementsByTagName("head")[0][a](s),s[i]?s[i].cssText=c:s[a](d.createTextNode(c))}(
...
),define("taoSyncloader\taoSync.bundle",function(){});
