...
helper=helpers.__||depth0&&depth0.__)?helper.call(depth0,"Cancel",options):helperMissing.call(depth0,"__","Cancel",options)))+'\n            </span>\n\n            <span class="btn-warning force-terminate-button">\n                <span class="icon-warning"></span>\n                '+escapeExpression((options={hash:{},data:data},(helper=helpers.__||depth0&&depth0.__)?helper.call(depth0,"Force end and synchronize",options):helperMissing.call(depth0,"__","Force end and synchronize",options)))+"\n            </span>\n        </div>\n    <div>\n</div>"})}),define("taoSync/component/terminateExecutions/terminateExecutions",["i18n","lodash",
"core/dataProvider/request","ui/component","tpl!taoSync/component/terminateExecutions/notification"],function(__,_,request,component,listTpl){"use strict";return function($container,config){if(!_.isPlainObject(config))throw new TypeError("The configuration is required");if(null==config.terminateUrl)throw new TypeError("terminateUrl configuration parameter is required");if(null==config.csrfToken)throw new TypeError("CSRF token is missing");return component({}).setTemplate(listTpl).on("init",function(){config.activeExecutions=[],config.aggregatedData={},config.notificationMessage=(1<config.data.length?__("There are %s assessments in progress",
config.data.length):__("There is one assessment in progress"))+", "+__("please ensure that everyone has completed the assessment before proceeding."),_.forEach(config.data,function(execution){config.activeExecutions.push(execution.execution_id),config.aggregatedData[execution.context_id]=config.aggregatedData[execution.context_id]||{total:0},config.aggregatedData[execution.context_id].label=execution.label,config.aggregatedData[execution.context_id].total++}),this.config=config}).on("render",function(){var self=this;$container.find(".cancel-button").on("click",function(e){e.preventDefault(),self.trigger("terminationCanceled"),self.destroy()}),
$container.find(".force-terminate-button").on("click",function(e){e.preventDefault(),function(){var requestData={executionsId:config.activeExecutions};return requestData[config.csrfToken.name]=config.csrfToken.token,request(config.terminateUrl,requestData,"POST")}().then(function(response){self.trigger("terminationSucceeded"),self.destroy()}).catch(function(error){self.trigger("terminationFailed")})}),self.trigger("dialogRendered")}).init(config)}}),define("taoSync/component/readinessDashboard/readinessDashboard",["i18n","lodash","core/eventifier","core/promise","ui/dashboard","core/dataProvider/request","util/url"],function(__,_,eventifier,Promise,dashboard
,request,urlHelper){"use strict";return function($container){return eventifier({performChecks:function(){var self=this;new Promise(function(resolve){request(urlHelper.route("getMachineChecks","Synchronizer","taoSync")).then(function(data){resolve(data)})}).then(function(data){self.dashboard.toggleLoadingBar(!1),self.dashboard.renderMetrics(data)})},render:function(){return this.dashboard=dashboard({renderTo:$container,loading:!0,warningText:!1,layoutType:"list"}),this.performChecks(),this},destroy:function(){return this.dashboard&&this.dashboard.destroy(),this.trigger("destroy"),this}})}}),define("taoSync/controller/synchronizer/index",["i18n","jquery",
"lodash","moment","core/dataProvider/request","util/url","ui/taskQueue/taskQueueModel","layout/loading-bar","taoSync/component/terminateExecutions/terminateExecutions","taoSync/component/readinessDashboard/readinessDashboard","core/promise","ui/filesender"],function(__,$,_,moment,request,urlHelper,taskQueueModelFactory,loadingBar,terminateExecutionsDialogFactory,readinessDashboardFactory,Promise){"use strict";var tq_api="TaskQueueWebApi",tq_ext="taoTaskQueue",webservices={get:urlHelper.route("get",tq_api,tq_ext),archive:urlHelper.route("archive",tq_api,tq_ext),all:urlHelper.route("getAll",tq_api,tq_ext),download:urlHelper.route("download",tq_api,tq_ext),
lastTask:urlHelper.route("lastTask","Synchronizer","taoSync"),activeSessions:urlHelper.route("activeSessions","Synchronizer","taoSync"),terminateExecutions:urlHelper.route("terminateExecutions","TerminateExecution","taoSync"),exportSyncData:urlHelper.route("createTask","Exporter","taoSync"),importSyncData:urlHelper.route("createTask","Importer","taoSync")},defaultSyncMessages={error:__("The synchronization process has failed."),progress:__("Synchronization in progress."),success:__("Success! The synchronization process has been finished.")};return{start:function(){var readinessDashboard,terminateExecutionsDialog,_msg,$container=$("#tao-sync-container"),
$syncForm=$container.find("form.sync-form"),$syncFormFields=($container.find("form.sync-form"),$syncForm.find('input:not([type="file"]), select')),$launchButton=$syncForm.find('button[data-control="launch"]'),$spinner=$syncForm.find(".feedback-info .icon-loop"),timeFields={$enqueued:$syncForm.find(".enqueue-time"),$updated:$syncForm.find(".update-time"),$completed:$syncForm.find(".complete-time")},msg=((_msg={$all:$syncForm.find(".msg")}).$all.each(function(){var $currentMsg=$(this);_msg["$"+$currentMsg.data("type")]=$currentMsg}),_msg),taskQueue=taskQueueModelFactory({url:{get:webservices.get,archive:webservices.archive,all:webservices.all,
download:webservices.download},pollSingleIntervals:[{iteration:Number.MAX_SAFE_INTEGER,interval:2e3}],pollAllIntervals:[{iteration:1,interval:8e3},{iteration:0,interval:5e3}]}).on("pollSingleFinished",function(taskId,taskData){"completed"===taskData.status?"oat\\taoSync\\scripts\\tool\\Export\\ExportSynchronizationPackage"===taskData.taskName?(setState("success",__("Synchronization data is ready to be downloaded. Downloading process will start in the background.")),taskQueue.download(taskId)):"oat\\taoSync\\scripts\\tool\\Import\\ImportSynchronizationPackage"===taskData.taskName?setState("success",__("Synchronization data has been imported."),!0):(setState(
"success"),updateTime(taskData),setHistoryTime(taskData.updatedAt,"$completed")):"failed"===taskData.status&&setState("error")}).on("pollSingle",function(taskId,taskData){updateTime(taskData)}).on("error",function(error){error.response&&400<=error.response.errorCode&&error.response.errorCode<500?setState("error",error.response.errorMsg):setState("error")});function getData(){var data={label:"Data Synchronization"};return $syncFormFields.each(function(){data[this.name]=this.value}),data}function toggleLaunchButtonState(){var isValid=!0;$syncFormFields.each(function(){return isValid=this.validity.valid}),isValid?$launchButton.removeAttr("disabled"
):$launchButton.attr("disabled","disabled")}function setState(state,message,hideDashboard){var statusClassName=".status-"+state;$container.find(statusClassName+" .messages p span:first-child").text(message||defaultSyncMessages[state]),$container.removeClass(function(index,className){return(className.match(/(^|\s)state-\S+/g)||[]).join(" ")}),$spinner["progress"===state?"addClass":"removeClass"]("spinner-icon"),$container.addClass("state-"+state),msg.$all.hide(),"success"!==state&&"error"!==state||hideDashboard?"progress"===state&&(readinessDashboard&&readinessDashboard.destroy(),readinessDashboard=null):function(){var $dashboardContainer=$container.find(
"#dashboard-container");readinessDashboard=readinessDashboardFactory($dashboardContainer).render()}()}function updateTime(taskData){setTime(taskData.createdAt,"$enqueued"),setTime(taskData.createdAt+taskData.createdAtElapsed,"$updated")}function setTime(timestamp,type){timeFields[type]&&timeFields[type].text(moment.unix(timestamp).format("LTS")),msg[type]&&msg[type].show()}function setHistoryTime(timestamp,type){setTime(timestamp,type),$container.addClass("history")}function checkActiveSessions(){return request(webservices.activeSessions).then(function(data){return terminateExecutionsDialog&&(terminateExecutionsDialog.destroy(),terminateExecutionsDialog=null
),!(Array.isArray(data.activeSessionsData)&&0<data.activeSessionsData.length)||function(data){var $terminateActionContainer=$container.find(".terminate-action"),$syncActionContainer=$container.find(".sync-action"),dialogConfig={data:data.activeSessionsData,csrfToken:data.token,terminateUrl:webservices.terminateExecutions};return new Promise(function(resolve,reject){terminateExecutionsDialog=terminateExecutionsDialogFactory($terminateActionContainer,dialogConfig).on("dialogRendered",function(){$syncActionContainer.toggle(),$terminateActionContainer.toggle()}).on("terminationCanceled",function(){$terminateActionContainer.toggle(),$syncActionContainer.toggle(),
resolve(!1)}).on("terminationFailed",function(){reject()}).on("terminationSucceeded",function(){$terminateActionContainer.toggle(),$syncActionContainer.toggle(),resolve(!0)}).render($terminateActionContainer)})}(data)})}$container.find(".fb-container").removeClass("viewport-hidden"),loadingBar.start(),$syncFormFields.on("keyup paste blur",toggleLaunchButtonState),toggleLaunchButtonState(),$syncForm.on("submit",function(e){var action=this.action;e.preventDefault(),setState("progress"),$container.removeClass("active"),checkActiveSessions().then(function(canStartSynchronization){canStartSynchronization?taskQueue.create(action,getData()):setState("form")}
