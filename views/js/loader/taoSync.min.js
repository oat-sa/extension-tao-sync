...
checkActiveSessions().then(function(canStartSynchronization){canStartSynchronization?taskQueue.create(action,getData()):setState("form")}).catch(function(){setState("error")})}),$syncForm.find('button[data-control="close"]').on("click",function(e){e.preventDefault(),setState("form")}),$syncForm.find('a[data-control="export"]').on("click",function(e){setState("progress",__("Preparation of synchronization data is in progress.")),checkActiveSessions().then(function(canStartSynchronization){canStartSynchronization?taskQueue.create(urlHelper.build(webservices.exportSyncData,getData())):setState("form")}).catch(function(){setState("error")})}),$syncForm.find(
'input[data-control="import"]').on("change",function(e){e.target.files[0]&&(setState("progress",__("Import of synchronization data is in progress.")),$syncForm.sendfile({url:webservices.importSyncData,loaded:function(response){taskQueue.pollSingle(response.data.task.id)},failed:function(){setState("error")}}))}),request(webservices.lastTask).then(function(currentTask){if(currentTask&&currentTask.status)switch(currentTask.status){case"failed":setState("error");break;case"completed":setState("form"),updateTime(currentTask),setHistoryTime(currentTask.updatedAt,"$completed");break;default:setState("progress"),updateTime(currentTask),taskQueue.pollSingle(
currentTask.id)}else setState("form");loadingBar.stop()}).catch(function(){setState("error")})}}}),define("taoSync/loader/taoSync.bundle",function(){});
