...
download:webservices.download},pollSingleIntervals:[{iteration:Number.MAX_SAFE_INTEGER,interval:2e3}],pollAllIntervals:[{iteration:1,interval:8e3},{iteration:0,interval:5e3}]}).on("pollSingleFinished",function(taskId,taskData){"completed"===taskData.status?"oat\\taoSync\\scripts\\tool\\Export\\ExportSynchronizationPackage"===taskData.taskName?(setState("form"),taskQueue.download(taskId)):(setState("success"),updateTime(taskData),setHistoryTime(taskData.updatedAt,"$completed")):"failed"===taskData.status&&setState("error")}).on("pollSingle",function(taskId,taskData){updateTime(taskData)}).on("error",function(error){
error.response&&400<=error.response.errorCode&&error.response.errorCode<500?setState("error",error.response.errorMsg):setState("error")});function getData(){var data={label:"Data Synchronization"};return $syncFormFields.each(function(){data[this.name]=this.value}),data}function toggleLaunchButtonState(){var isValid=!0;$syncFormFields.each(function(){return isValid=this.validity.valid}),isValid?$launchButton.removeAttr("disabled"):$launchButton.attr("disabled","disabled")}function setState(state,message){var statusClassName=".status-"+state;$container.find(statusClassName+" .messages p span:first-child").text(message||defaultSyncMessages[state]),
$container.removeClass(function(index,className){return(className.match(/(^|\s)state-\S+/g)||[]).join(" ")}),$spinner["progress"===state?"addClass":"removeClass"]("spinner-icon"),$container.addClass("state-"+state),msg.$all.hide(),"success"===state||"error"===state?function(){var $dashboardContainer=$container.find("#dashboard-container");readinessDashboard=readinessDashboardFactory($dashboardContainer).render()}():"progress"===state&&(readinessDashboard&&readinessDashboard.destroy(),readinessDashboard=null)}function updateTime(taskData){setTime(taskData.createdAt,"$enqueued"),setTime(taskData.createdAt+taskData.createdAtElapsed,"$updated")}function setTime(
timestamp,type){timeFields[type]&&timeFields[type].text(moment.unix(timestamp).format("LTS")),msg[type]&&msg[type].show()}function setHistoryTime(timestamp,type){setTime(timestamp,type),$container.addClass("history")}function checkActiveSessions(){return request(webservices.activeSessions).then(function(data){return terminateExecutionsDialog&&(terminateExecutionsDialog.destroy(),terminateExecutionsDialog=null),!(Array.isArray(data.activeSessionsData)&&0<data.activeSessionsData.length)||function(data){var $terminateActionContainer=$container.find(".terminate-action"),$syncActionContainer=$container.find(".sync-action"),dialogConfig={
...
$container.find(".fb-container").removeClass("viewport-hidden"),loadingBar.start(),$syncFormFields.on("keyup paste blur",toggleLaunchButtonState),toggleLaunchButtonState(),$syncForm.on("submit",function(e){var action=this.action;e.preventDefault(),setState("progress"),$container.removeClass("active"),checkActiveSessions().then(function(canStartSynchronization){canStartSynchronization?taskQueue.create(action,getData()):setState("form")}).catch(function(){setState("error")})}),$syncForm.find('button[data-control="close"]').on("click",function(e){e.preventDefault(),setState("form")}),$syncForm.find('a[data-control="export"]').on("click",function(e){setState(
"progress",__("Preparation of synchronization data is in progress.")),checkActiveSessions().then(function(canStartSynchronization){canStartSynchronization?taskQueue.create(urlHelper.build(webservices.exportSyncData,getData())):setState("form")}).catch(function(){setState("error")})}),$syncForm.find('input[data-control="import"]').on("change",function(e){e.target.files[0]&&(setState("progress"),$syncForm.sendfile({url:webservices.importSyncData,loaded:function(response){taskQueue.pollSingle(response.data.task.id)},failed:function(){setState("error")}}))}),request(webservices.lastTask).then(function(currentTask){if(currentTask&&currentTask.status)switch(
currentTask.status){case"failed":setState("error");break;case"completed":setState("form"),updateTime(currentTask),setHistoryTime(currentTask.updatedAt,"$completed");break;default:setState("progress"),updateTime(currentTask),taskQueue.pollSingle(currentTask.id)}else setState("form");loadingBar.stop()}).catch(function(){setState("error")})}}}),define("taoSync/loader/taoSync.bundle",function(){});
